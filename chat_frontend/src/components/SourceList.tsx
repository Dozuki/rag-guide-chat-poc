...
const buildGuideHref = (
	...
	const normalizedPath = url && url.startsWith('/') ? url : url ? `/${url}` : undefined
	const siteBase = DOZUKI_BASE_URL || (siteSlug ? `https://${siteSlug}.dozuki.com` : undefined)
...
const SourceList = ({ sources = [], guides = [], numContexts }: SourceListProps) => {
	const uniqueSources = useMemo(() => Array.from(new Set(sources)).filter(Boolean), [sources])
				...
				if (href) {
					nodes.push(
						<a
							...
							rel='noreferrer'
						>
							<span className='sources__item-title'>Guide {parsed.guideId}</span>
		...
		>
			<AccordionSummary
				...
				sx={{
					...
					'py': 0,
					'minHeight': 'unset',
					'& .MuiAccordionSummary-expandIconWrapper': { color: 'rgba(148, 163, 184, 0.9)' },
					'& .MuiAccordionSummary-expandIconWrapper.Mui-expanded': { transform: 'none' },
					'& .MuiAccordionSummary-expandIconWrapper .MuiSvgIcon-root': { transition: 'transform 200ms' },
					'& .MuiAccordionSummary-expandIconWrapper.Mui-expanded .MuiSvgIcon-root': { transform: 'rotate(180deg)' },
						...
						(() => {
							const guideId =
								typeof guide.guide_id === 'number'
									? guide.guide_id
									: Number.parseInt(String(guide.guide_id ?? Number.NaN), 10)
							const sourceMeta = Number.isNaN(guideId) ? undefined : parsedSourceMeta.get(guideId)
							...
							if (!href) {
								return (
									...
									>
										...
										{displayUrl ? <span className='sources__item-subtitle'>{displayUrl}</span> : null}
									</div>
								)
							}
